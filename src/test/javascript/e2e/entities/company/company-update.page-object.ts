...
export default class CompanyUpdatePage {
  pageTitle: ElementFinder = element(by.id('geekwaycoreApp.company.home.createOrEditLabel'));
  saveButton: ElementFinder = element(by.id('save-entity'));
  cancelButton: ElementFinder = element(by.id('cancel-save'));
  nameInput: ElementFinder = element(by.css('input#company-name'));
  identifierInput: ElementFinder = element(by.css('input#company-identifier'));
  ...
  async enterData() {
    ...
    await waitUntilDisplayed(this.saveButton);
    await this.setIdentifierInput('identifier');
    expect(await this.getIdentifierInput()).to.match(/identifier/);
    await waitUntilDisplayed(this.saveButton);
